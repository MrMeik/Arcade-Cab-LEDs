...
LedLine leds[NUM_LED_LINES] = {
    { NUM_LEDS_1, LED_PIN_1, leds_1 },
    { NUM_LEDS_2, LED_PIN_2, leds_2 },
};
...
void setup() {
    ...
    FastLED.addLeds<WS2812B, LED_PIN_1, GRB>(leds[0].leds, leds[0].num_leds);
    FastLED.addLeds<WS2812B, LED_PIN_2, GRB>(leds[1].leds, leds[1].num_leds);
...
#define ANIM_OFF 0
#define ANIM_SOLID 1
#define ANIM_SOLID_FLASH 2
#define ANIM_RAINBOW 3
#define ANIM_PULSE 4

int animationState = ANIM_PULSE;
...
void loop() {
    ...
    switch (animationState) {
        case ANIM_PULSE:
            ...
            for (int lineIndex = 0; lineIndex < NUM_LED_LINES; lineIndex++) {
                LedLine led = leds[lineIndex];

                for (int i = led.num_leds - 1; i > 0; i--) {
                    led.leds[i] = led.leds[i - 1];
                }

                led.leds[0] = newPixelColor;
                //FastLED.addLeds<WS2812B, leds[i].led_pin, GRB>(leds[i].leds, leds[i].num_leds);
            }
            break;
        case ANIM_RAINBOW:
            for (int lineIndex = 0; lineIndex < NUM_LED_LINES; lineIndex++) {
                LedLine led = leds[lineIndex];
                fill_rainbow(led.leds, led.num_leds, animationOffset);
            }
    ...
    fill_rainbow(leds_1, NUM_LEDS_1, 0);
    fill_rainbow(leds_2, NUM_LEDS_2, 0);
    FastLED.show();
